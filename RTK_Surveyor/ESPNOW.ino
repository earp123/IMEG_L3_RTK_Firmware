...
void espnowGUIcommandRx(const uint8_t *mac, const uint8_t *incomingData, int len)
{
  Serial.write(incomingData, len);
  Serial.println();

  memcpy(&tx_cmd_packet, incomingData, len);
}
...
void espnowStart()
{
  ...
  esp_now_register_recv_cb(espnowGUIcommandRx);
...
void espnowProcessGUI(byte incoming)
{
#ifdef COMPILE_ESPNOW
  if (espnowState == ESPNOW_PAIRED)
  {
    //Move this byte into ESP NOW to send buffer
    espnowOutgoing[espnowOutgoingSpot++] = incoming;//espnowOutgoing[250]; //ESP NOW has max of 250 characters
    espnowLastAdd = millis();

    if (espnowOutgoingSpot == sizeof(GUIpacket))//change sizeof(espnowOutgoing) to GUI imfo packet size
    {
      ...
      Serial.print("GUI bytes sent: "); Serial.println(sizeof(GUIpacket));
