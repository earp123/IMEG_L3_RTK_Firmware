...
//~SWR~ added this to test the additional BT function for separate data
uint16_t hr_test_word = 40;
...
float lux; //~SWR~ variable to hold the reading from VEML7700
bool VEMLstarted = false;

//Moved these variables to global from updateRTC()
int hour;
int minute;
int second;
...
void loop()
{
  ...
  if (/*settings.enablePrintPosition && */((millis() - lastPrintPosition) > 5000))//SWR overriding this for the time being
  {
...
void updateVEMLreading()
{
  if (VEMLstarted)
  {
    lux = veml.readLux(VEML_LUX_READING); //returns -1 if lighting environment is unstable
    if (lux>0) 
      {Serial.print("Lux = "); Serial.println(lux);}
    else 
      Serial.println("Unstable Ligting Environment");
  }              
  else 
    Serial.println("VEML sensor not started");
...
void connectGUI()
{
  ...
  uint8_t GUI_peer[] = {0x7C, 0xDF, 0xA1, 0x4D, 0xD1, 0x23}; //Sam's TFT GUI Mac

  if (esp_now_is_peer_exist(GUI_peer) == true)
    log_d("Peer already exists");
  else
  {
    //Add new peer to system
    espnowAddPeer(GUI_peer);

    //Record this MAC to peer list
    memcpy(settings.espnowPeers[settings.espnowPeerCount], GUI_peer, 6);
    settings.espnowPeerCount++;
    settings.espnowPeerCount %= ESPNOW_MAX_PEERS;
    recordSystemSettings();
    Serial.println("GUI Connected via ESP NOW");
  }
...
void updateGUI()
{
  unsigned char GUI_a[sizeof(GUIpacket)];

  GUIpacket.batt        = battLevel;
  GUIpacket.SatsInView  = numSV;
  GUIpacket.HorizAcc    = horizontalAccuracy;
  GUIpacket.lux         = lux;
  GUIpacket.latitude    = latitude;
  GUIpacket.longitude   = longitude;

  GUIpacket.hour_s = rtc.getHour();
  GUIpacket.min_s  = rtc.getMinute();
  GUIpacket.sec_s  = rtc.getSecond();

  if (bluetoothGetState() == BT_CONNECTED)
    GUIpacket.bt_connected = true;
  else 
    GUIpacket.bt_connected = false;

  //Transmit
  memcpy(GUI_a, &GUIpacket, sizeof(GUIpacket));
  for (int i = 0; i < sizeof(GUIpacket); i++) 
  {
    espnowProcessGUI(GUI_a[i]);
  }
