...
void F9PSerialReadTask(void *e)
{
  ...
  while (true)
  {
    ...
    else if (btBytesToSend > 0)
    {
      ...
      //~SWR~ will need to modify this transaction to inlcude light sensor data
      ...
      if (btBytesToSend > 0)
      {
        ...
        if (systemState >= STATE_BASE_NOT_STARTED && systemState <= STATE_BASE_FIXED_TRANSMITTING)//~SWR~ won't be including base mode
  ...
  while (true)
  {
    ...
    */
    ...
    else if (productVariant == RTK_EXPRESS || productVariant == RTK_EXPRESS_PLUS) //Express: Check both of the momentary switches
    {
      ...
      {//~SWR~ this section controls the setup and power button tasks
        switch (systemState)
        {
          ...
          case STATE_ESPNOW_PAIRING:
            ...
            /*~SWR~ 
            TODO call VEML7700 function here on button press
            if (VEMLstarted){
              ...
              if (lux>0) 
              {
                ...
                uint16_t lux_word = (uint16_t) lux;
                if (bluetoothWriteHR(lux_word)) // SWR deleted this function, need to rewrite
                {
                  Serial.print(lux_word); Serial.println(" Lux written to HR Measurement");
                }
                else
                {
                  Serial.println("HR Write Failed");
                }  
              }
              else Serial.println("Unstable Ligting Environment");
            }              
            else Serial.println("VEML7700 not found");
              //TODO maybe display the lux value on the display
              */
            break;
